...
FaceIdentification::FaceIdentification(std::string model_path)
{
	recognizer = new Recognizer(model_path.c_str());
}
...
uint8_t FaceIdentification::CropFace(const ImageData &src_image,
  ...
  float point_data[10];
  for (int i = 0; i < 5; ++i) {
	  point_data[i * 2] = (float)llpoint[i].x;
	  point_data[i * 2 + 1] = (float)llpoint[i].y;
  }
