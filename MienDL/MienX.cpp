...
bool MienDL::detect(cv::Mat & cvmat, std::vector<Landmark> &landmarks) {
...
bool MienDL::beone(std::vector<Landmark> &landmarks, Landmark &landmark) {
	int n = landmarks.size();	if (n < 1) return false;
	for (int i=0; i<n; i++)
...
//void MienDL::showLandmark(cv::Mat& img, Landmark& landmark) {
